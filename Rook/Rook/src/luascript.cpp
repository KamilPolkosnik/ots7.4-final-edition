...
void LuaScriptInterface::pushInstantSpell(lua_State* L, const InstantSpell& spell)
{
	...
	setField(L, "learnable", spell.isLearnable());
...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnum(CONST_SLOT_BACKPACK_1)
	registerEnum(CONST_SLOT_BACKPACK_2)
	registerEnum(CONST_SLOT_BACKPACK_3)
...
int LuaScriptInterface::luaPlayerShowTextDialog(lua_State* L)
{
	...
	if (!canWrite) {
		...
		if (outText.empty() && fromUserdata && item) {
			outText = item->getText();
		}

		player->setWriteItem(nullptr);
		player->sendTextWindow(itemId, outText);
		pushBoolean(L, true);
		return 1;
	}
	...
	player->sendTextWindow(item, length, true);
	pushBoolean(L, true);
	return 1;
}
...
int LuaScriptInterface::luaPlayerGetInstantSpells(lua_State* L)
{
	...
	for (auto& spell : g_spells->getInstantSpells()) {
		...
		const auto& vocMap = spell.second.getVocMap();
		const bool vocationMatch = vocMap.find(player->getVocationId()) != vocMap.end();
		const bool learnedByPlayer = player->hasLearnedInstantSpell(spell.second.getName());
		if (vocationMatch || learnedByPlayer) {
			spells.push_back(&spell.second);
		}
	}
