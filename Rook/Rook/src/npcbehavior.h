...
public:
	...
	const std::list<ShopInfo>& getShopItems() const {
		return shopItems;
	}
	bool hasShopItems() const {
		return !shopItems.empty();
	}
	bool handleShopTrade(Player* player, uint16_t itemId, uint8_t subType, uint8_t amount, bool ignoreCap, bool inBackpacks);
	bool handleShopSell(Player* player, uint16_t itemId, uint8_t subType, uint8_t amount, bool ignoreEquipped);
...
private:
	...
	void collectShopItem(const NpcBehaviourPtr& behaviour);
	bool getConstNumber(const NpcBehaviourNodePtr& node, int32_t& out) const;
	bool getConstPrice(const NpcBehaviourNodePtr& node, int32_t& out) const;
	void addShopItem(uint16_t itemId, int32_t subType, uint32_t buyPrice, uint32_t sellPrice);
	bool isSellShopEntry(const NpcBehaviourPtr& behaviour) const;
