...
bool Items::loadFromXml()
{
	...
	// Client id overrides for mismatched client.dat (letters/labels, etc.)
	static const std::unordered_map<uint16_t, uint16_t> clientIdOverrides = {
		{2597, 3505}, // letter
		{2599, 3507}, // label
	};
	for (const auto& [serverId, clientId] : clientIdOverrides) {
		if (serverId >= items.size() || items[serverId].id == 0) {
			std::cout << "[Warning - Items::loadFromXml] clientId override skipped, missing server id: " << serverId << std::endl;
			continue;
		}
		uint16_t existing = clientIdToServerIdMap.getServerId(clientId);
		if (existing != 0 && existing != serverId) {
			std::cout << "[Warning - Items::loadFromXml] clientId override " << clientId
				<< " was mapped to server id " << existing << ", forcing to " << serverId << std::endl;
		}
		items[serverId].clientId = clientId;
		clientIdToServerIdMap.set(clientId, serverId);
	}
