...
bool addShopItemToPlayerOrGround(Player* player, Item* item)
{
	...
	// Extra safety fallback: force drop on player's tile if first add fails.
	ret = g_game.internalAddItem(player->getTile(), item, INDEX_WHEREEVER, FLAG_NOLIMIT);
...
bool NpcBehavior::getConstNumber(const NpcBehaviourNodePtr& node, int32_t& out) const
{
	if (!node) {
		return false;
	}

	if (node->type == BEHAVIOUR_TYPE_NUMBER) {
		out = node->number;
		return true;
	}
...
bool NpcBehavior::getConstPrice(const NpcBehaviourNodePtr& node, int32_t& out) const
{
	if (!node) {
		return false;
	}
	...
	if (node->type == BEHAVIOUR_TYPE_OPERATION && node->number == BEHAVIOUR_OPERATOR_MULTIPLY) {
		// Accept patterns like 60 * %1, %1 * 60, 5 * Amount, Amount * 5.
		int32_t value = 0;
		if (node->left && node->right) {
			if (node->left->type == BEHAVIOUR_TYPE_NUMBER &&
			    (node->right->type == BEHAVIOUR_TYPE_MESSAGE_COUNT || node->right->type == BEHAVIOUR_TYPE_AMOUNT)) {
				value = node->left->number;
				out = value;
				return true;
			}
			if (node->right->type == BEHAVIOUR_TYPE_NUMBER &&
			    (node->left->type == BEHAVIOUR_TYPE_MESSAGE_COUNT || node->left->type == BEHAVIOUR_TYPE_AMOUNT)) {
				value = node->right->number;
				out = value;
				return true;
			}
		}
...
void NpcBehavior::addShopItem(uint16_t itemId, int32_t subType, uint32_t buyPrice, uint32_t sellPrice)
{
	const ItemType& it = Item::items[itemId];
	if (it.id == 0 || (buyPrice == 0 && sellPrice == 0)) {
		return;
	}

	int32_t actualSubType = 0;
	if (it.isFluidContainer() || it.isSplash() || it.isKey()) {
		actualSubType = subType;
	}

	for (ShopInfo& info : shopItems) {
		if (info.itemId == itemId && info.subType == actualSubType) {
			if (buyPrice != 0) {
	...
	ShopInfo info(itemId, actualSubType, buyPrice, sellPrice, it.name);
	shopItems.push_back(info);
}

bool NpcBehavior::isSellShopEntry(const NpcBehaviourPtr& behaviour) const
{
	for (const NpcBehaviourConditionPtr& condition : behaviour->conditions) {
		if (condition && condition->type == BEHAVIOUR_TYPE_STRING && condition->string == "sell") {
			return true;
		}
	}
	return false;
...
void NpcBehavior::collectShopItem(const NpcBehaviourPtr& behaviour)
{
	int32_t itemId = -1;
	int32_t price = -1;
	int32_t dataValue = 0;
	bool hasData = false;
	int32_t deleteItemId = -1;
	bool hasCreateMoney = false;
	...
	for (const NpcBehaviourActionPtr& action : behaviour->actions) {
		...
		switch (action->type) {
			case BEHAVIOUR_TYPE_ITEM: {
				int32_t value = -1;
				if (getConstNumber(action->expression, value)) {
					itemId = value;
				}
				break;
			}
			case BEHAVIOUR_TYPE_PRICE: {
				int32_t value = -1;
				if (getConstPrice(action->expression, value)) {
					price = value;
				}
				break;
			}
	...
	if (itemId > 0 && price > 0) {
		const bool sellEntry = isSellShopEntry(behaviour);
		const uint32_t buyPrice = sellEntry ? 0 : static_cast<uint32_t>(price);
		const uint32_t sellPrice = sellEntry ? static_cast<uint32_t>(price) : 0;
		addShopItem(static_cast<uint16_t>(itemId), hasData ? dataValue : 0, buyPrice, sellPrice);
		return;
	}

	// Fallback for NPCs using "deposit" dialogs (e.g., empty vials):
	// Topic lines often have only Delete(<item>) + CreateMoney + Price=Amount*X.
	if (deleteItemId > 0 && hasCreateMoney && price > 0) {
		addShopItem(static_cast<uint16_t>(deleteItemId), hasData ? dataValue : 0, 0, static_cast<uint32_t>(price));
	}
}

bool NpcBehavior::handleShopTrade(Player* player, uint16_t itemId, uint8_t subType, uint8_t amount, bool /*ignoreCap*/, bool /*inBackpacks*/)
{
	if (!player || shopItems.empty()) {
		return false;
	}
	...
	const ShopInfo* shopInfo = nullptr;
	for (const ShopInfo& info : shopItems) {
		if (info.itemId != itemId || info.buyPrice == 0) {
			continue;
		}
		if ((it.isFluidContainer() || it.isSplash()) && info.subType != subType) {
			continue;
		}
		shopInfo = &info;
		break;
	...
	uint64_t totalCost = static_cast<uint64_t>(shopInfo->buyPrice) * amount;
	if (player->getMoney() < totalCost) {
		player->sendCancelMessage("You don't have enough money.");
		return true;
	}

	if (!g_game.removeMoney(player, totalCost)) {
		player->sendCancelMessage("You don't have enough money.");
		return true;
	}

	uint32_t giveAmount = amount;
	if (it.stackable) {
		while (giveAmount > 0) {
			int32_t stackCount = std::min<int32_t>(100, giveAmount);
			Item* item = Item::CreateItem(it.id, stackCount);
			if (!item) {
				break;
			}

			if (!addShopItemToPlayerOrGround(player, item)) {
				delete item;
				player->sendCancelMessage("You don't have enough room.");
				return true;
			}
			giveAmount -= stackCount;
		}
	} else {
		for (uint32_t i = 0; i < giveAmount; ++i) {
			int32_t itemSubType = (it.isFluidContainer() || it.isSplash()) ? shopInfo->subType : 1;
			Item* item = Item::CreateItem(it.id, itemSubType);
			if (!item) {
				break;
			}
			if (it.isKey()) {
				if (shopInfo->subType > 0) {
					item->setActionId(static_cast<uint16_t>(shopInfo->subType));
					std::cout << "[KeyDebug] Shop created key itemid=" << it.id
					          << " actionid=" << shopInfo->subType
					          << " player=" << player->getName() << std::endl;
				} else {
					std::cout << "[KeyDebug] Shop created key itemid=" << it.id
					          << " but subType<=0 (no actionid). player="
...
bool NpcBehavior::handleShopSell(Player* player, uint16_t itemId, uint8_t subType, uint8_t amount, bool ignoreEquipped)
{
	if (!player || shopItems.empty()) {
		return false;
	}
	...
	const ShopInfo* shopInfo = nullptr;
	for (const ShopInfo& info : shopItems) {
		if (info.itemId != itemId || info.sellPrice == 0) {
			continue;
		}
		if ((it.isFluidContainer() || it.isSplash()) && info.subType != subType) {
			continue;
	...
	if (!shopInfo) {
		player->sendCancelMessage("This NPC is not buying this item.");
		return true;
	}

	const int32_t removeSubType = (it.isFluidContainer() || it.isSplash()) ? static_cast<int32_t>(subType) : -1;
	const uint32_t ownedAmount = player->getItemTypeCount(itemId, removeSubType);
	if (ownedAmount < amount) {
		player->sendCancelMessage("You don't have that item.");
		return true;
	}

	if (!player->removeItemOfType(itemId, amount, removeSubType, ignoreEquipped)) {
		player->sendCancelMessage("You don't have that item.");
		return true;
	}

	const uint64_t totalPrice = static_cast<uint64_t>(shopInfo->sellPrice) * amount;
...
void NpcBehavior::checkAction(const NpcBehaviourActionPtr& action, Player* player, std::string& message)
{
	switch (action->type) {
		...
		case BEHAVIOUR_TYPE_CREATE: {
			...
			if (it.stackable) {
				...
				for (int32_t i = 0; i < std::max<int32_t>(1, amount); i++) {
					...
					if (it.isKey()) {
						if (data > 0) {
							item->setActionId(static_cast<uint16_t>(data));
							std::cout << "[KeyDebug] NPC created key itemid=" << itemId
							          << " actionid=" << data
							          << " player=" << player->getName() << std::endl;
						} else {
							std::cout << "[KeyDebug] NPC created key itemid=" << itemId
							          << " but data<=0 (no actionid). player="
							          << player->getName() << std::endl;
						}
					}
		...
		case BEHAVIOUR_TYPE_BURNING: {
			...
			conditionDamage->addDamage(std::max<int32_t>(1, cycles), 2000, -std::abs(count));
