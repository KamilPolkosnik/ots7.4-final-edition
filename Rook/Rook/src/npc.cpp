...
namespace {
	constexpr int32_t BEHAVIOR_SHOP_BUY = -2;
	constexpr int32_t BEHAVIOR_SHOP_SELL = -3;

	bool isTradeRequestMessage(const std::string& text)
	{
		const std::string lower = asLowerCaseString(text);
		return lower.find("trade") != std::string::npos ||
		       lower.find("offer") != std::string::npos ||
		       lower.find("wares") != std::string::npos;
	}
}
...
void Npc::reload()
{
	...
	if (!npcBehavior && hasSpectators && walkTicks > 0) {
		addEventWalk();
	}
...
bool Npc::loadFromXml()
{
	...
	if (behaviorFile) {
		...
		if (!npcBehavior->loadDatabase(behaviorFilename)) {
			std::cout << "[Error - Npc::loadFromXml] Failed to load npc behavior file: " << behaviorFilename << std::endl;
			delete npcBehavior;
			npcBehavior = nullptr;
		}
	}
...
void Npc::onRemoveCreature(Creature* creature, bool isLogout)
{
	...
	if (creature == this) {
		...
		if (npcBehavior && player->getID() == static_cast<uint32_t>(focusCreature)) {
                         ...
                         const Tile* oldTile, const Position& oldPos, bool teleport)
{
	...
	if (creature == this || creature->getPlayer()) {
		...
		if (creature != this) {
			...
			if (npcBehavior && player && player->getID() == static_cast<uint32_t>(focusCreature)) {
...
void Npc::onCreatureSay(Creature* creature, SpeakClasses type, const std::string& text)
{
	...
	if (player) {
		...
		if (npcBehavior && type == TALKTYPE_SAY) {
			if (npcBehavior->hasShopItems() && focusCreature == static_cast<int32_t>(player->getID()) &&
			    isTradeRequestMessage(text)) {
				openBehaviorShop(player);
			}
...
void Npc::onThink(uint32_t interval)
{
	...
	if (!npcBehavior && !isIdle && getTimeSinceLastMove() >= walkTicks) {
		addEventWalk();
	}
}
...
{
	if (callback == BEHAVIOR_SHOP_BUY && npcBehavior) {
		npcBehavior->handleShopTrade(player, itemId, count, amount, ignore, inBackpacks);
		player->sendSaleItemList();
		return;
	}

	if (callback == BEHAVIOR_SHOP_SELL && npcBehavior) {
		npcBehavior->handleShopSell(player, itemId, count, amount, ignore);
		player->sendSaleItemList();
		return;
	}
...
void Npc::openBehaviorShop(Player* player)
{
	if (!npcBehavior || !player) {
		return;
	}

	const auto& items = npcBehavior->getShopItems();
	if (items.empty()) {
		return;
	}

	player->closeShopWindow(false);
	addShopPlayer(player);
	player->setShopOwner(this, BEHAVIOR_SHOP_BUY, BEHAVIOR_SHOP_SELL);
