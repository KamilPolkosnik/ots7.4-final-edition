...
int64_t Creature::calculateToDoDelay()
{
	...
	if (toDoEntry.type == TODO_WALK) {
		const int64_t walkDelay = getWalkDelay();
		return walkDelay > 0 ? walkDelay : 0;
	} else if (toDoEntry.type == TODO_WAIT) {
		if (now >= toDoEntry.time) {
			return 0;
		}
		return toDoEntry.time - now;
	}
...
void Creature::addWalkToDo(Direction dir)
{
	...
	toDoEntry.function = [this, dir]() {
		g_game.internalMoveCreature(this, dir, FLAG_IGNOREFIELDDAMAGE);
	};
	toDoEntries.push_back(toDoEntry);
}
