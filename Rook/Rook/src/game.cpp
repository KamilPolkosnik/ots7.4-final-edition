...
namespace {
constexpr uint8_t CORPSE_PULSE_EFFECT_ID = 89;
...
constexpr uint32_t CORPSE_PULSE_EFFECT_LOOP_MS = 2000;
                                   ...
                                   Item* item, uint32_t count, Item** _moveItem, uint32_t flags /*= 0*/, Creature* actor/* = nullptr*/, Item* tradeItem/* = nullptr*/, const Position* fromPos /*= nullptr*/, const Position* toPos/*= nullptr*/)
{
	...
	if ((index == CONST_SLOT_LEFT || index == CONST_SLOT_RIGHT) && (item->getSlotPosition() & SLOTP_TWO_HAND)) {
		if (Player* toPlayer = dynamic_cast<Player*>(toCylinder)) {
