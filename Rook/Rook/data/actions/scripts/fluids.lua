...
local ITEM_POOL = rawget(_G, "POOL") or 2016
...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	local itemExUid = itemEx.uid or 0
	local itemExItemId = itemEx.itemid or 0
	local itemExType = itemEx.type or 0

	if samePosition(getCreaturePosition(cid), toPosition) then
		itemExUid = cid
	end
	...
	if(itemExUid == cid) then
		...
		if(item.type == TYPE_MANA_FLUID) then
			if(not doPlayerAddMana(itemExUid, math.random(55, 85))) then
				return false
			end

			doCreatureSay(itemExUid, "Aaaah...", TALKTYPE_SAY)
			doSendMagicEffect(toPosition, CONST_ME_MAGIC_BLUE)
		elseif(item.type == TYPE_LIFE_FLUID) then
			if(not doCreatureAddHealth(itemExUid, math.random(40, 70))) then
				return false
			end
	...
	if(not isCreature(itemExUid)) then
		if(item.type == TYPE_EMPTY) then
			if(isItemFluidContainer(itemExItemId) and itemExType ~= TYPE_EMPTY) then
				doChangeTypeItem(item.uid, itemExType)
				doChangeTypeItem(itemExUid, TYPE_EMPTY)
				return true
			end

			local tmp = casks[itemExItemId]
			if(tmp == nil) then
				tmp = getFluidSourceType(itemExItemId)
			end
		...
		local tmp = oilLamps[itemExItemId]
		if(item.type == TYPE_OIL and tmp ~= nil) then
			doTransformItem(itemExUid, tmp)
			doChangeTypeItem(item.uid, TYPE_EMPTY)
			return true
		end

		if(isItemFluidContainer(itemExItemId) and itemExType == TYPE_EMPTY) then
			doChangeTypeItem(itemExUid, item.type)
			doChangeTypeItem(item.uid, TYPE_EMPTY)
			return true
		end

		if(hasProperty(itemExUid, CONST_PROP_BLOCKSOLID)) then
			return false
		end
	end
