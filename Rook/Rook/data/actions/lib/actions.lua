...
-- BridgeRelocate is used by several switch/bridge scripts but isn't defined in the
-- actions interface by default. Keep a local implementation here so it is always
...
-- Transform (or remove) an item at a position after X seconds.
-- Used by lever/ground scripts to revert changes.
function doDecayItemTo(position, itemId, ticks)
	if not position or not itemId then
		return false
	end

	local delay = (ticks or 1) * 1000
	local pos = Position(position.x, position.y, position.z)
	local stackpos = position.stackpos or 1
...
-- Legacy quest scripts (main/quest.lua, rook/quest.lua, djinn war) call doAddQuestPoint(cid).
-- Some distributions don't define this function; keep a safe fallback to avoid runtime errors.
if not doAddQuestPoint then
	function doAddQuestPoint(cid, amount)
		...
		local storage = 90000
		local current = player:getStorageValue(storage)
		if current < 0 then
			current = 0
		end
		player:setStorageValue(storage, current + points)
		return true
	end
end
