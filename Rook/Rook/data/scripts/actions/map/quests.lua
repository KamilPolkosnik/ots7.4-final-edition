local chests = {
	...
	[1018] = {
		storageValue = 18,
		item = {id = 1950, count = 1, text = "Hardek *\nBozo *\nSam ****\nOswald \nPartos ***\nQuentin *\nTark ***\nHarsky ***\nStutch *\nFerumbras * \nFrodo ** \nNoodles **** \n"}, -- a book
	},
	[1019] = {
		storageValue = 19,
	...
	[1041] = {
		storageValue = 41,
		item = {id = 2152, count = 25}, -- a wand of cosmic energy
	},
	[1042] = {
		storageValue = 42,
	...
	[1160] = {
		storageValue = 160,
		item = {id = 1956, count = 1, text = "*You see a map of the surface of the Fields of Glory. There are many red lines and two golden points on it. You wonder what their meaning is.*"}, -- a map
	},
	[1161] = {
		storageValue = 161,
	...
	[1181] = {
		storageValue = 181,
		item = {id = 2263, charges = 3}, -- a spell rune
	},
	[1182] = {
		storageValue = 182,
		item = {id = 1987, count = 1}, -- a bag
	...
	[1255] = {
		storageValue = 255,
		item = {id = 1950, count = 1, text = "If you found this in a corpse behind a questdoor, its a bug. Please report it."}, -- a book
	},
	[1257] = {
		storageValue = 257,
	...
	[1332] = {
		storageValue = 332,
		item = {id = 2318, count = 1}, -- an explorer brooch
	},
}
...
function action.onUse(player, item, fromPosition, target, toPosition)	local itemId = item:getId()
	...
	elseif table.contains(closedLevelDoors, itemId) then
		if item.actionid > 0 and player:getLevel() >= item.actionid - actionIds.levelDoor or player:getGroup():getAccess() then
			...
			local requiredLevel = math.max(0, item.actionid - actionIds.levelDoor)
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Only the worthy may pass. (You need lvl " .. requiredLevel .. " to open this door)")
		end
		return true
	end
...
for aid in pairs(chests) do
	action:aid(aid)
end
action:aid(1203) -- Annihilator rewards handler
action:register()
