...
-- Compatibility helpers for newer script APIs (TVP/OTServ style)
if not Game then
	Game = {}
end

local function findItemInPosition(pos, itemId)
	if not pos then
		return nil
	end
	-- Prefer Tile API when available (handles ground items reliably)
	if Tile then
		local tile = Tile(pos)
		if tile then
	...
	-- Fallback: iterate stack positions
	local probe = {x = pos.x, y = pos.y, z = pos.z, stackpos = 0}
	for stack = 0, 20 do
		probe.stackpos = stack
		local thing = getThingfromPos(probe)
		if thing and thing.uid and thing.uid > 0 then
...
if not Game.removeItemInPosition then
	function Game.removeItemInPosition(pos, itemId)
		-- Prefer Tile API when available
		if Tile then
			local tile = Tile(pos)
			if tile then
				local item = tile:getItemById(itemId)
		...
		local thing = findItemInPosition(pos, itemId)
		if thing and thing.uid and thing.uid > 0 then
			return doRemoveItem(thing.uid, 1)
		end
		return false
	end
end

if not Game.transformItemInPosition then
	function Game.transformItemInPosition(pos, fromId, toId)
		-- Prefer Tile API when available
		if Tile then
			local tile = Tile(pos)
			if tile then
		...
		local thing = findItemInPosition(pos, fromId)
		if thing and thing.uid and thing.uid > 0 then
			return doTransformItem(thing.uid, toId)
		end
		return false
	end
end

if not Game.createItem then
	function Game.createItem(pos, itemId, count)
		local toPos = {x = pos.x, y = pos.y, z = pos.z}
		return doCreateItem(itemId, count or 1, toPos)
	end
end

if not Game.isItemInPosition then
	function Game.isItemInPosition(pos, itemId)
		if Tile then
			local tile = Tile(pos)
			if tile then
				local item = tile:getItemById(itemId)
		...
		local thing = findItemInPosition(pos, itemId)
		return thing ~= nil and thing.uid ~= nil and thing.uid > 0
	end
end

if not Game.sendMagicEffect then
	function Game.sendMagicEffect(pos, effect)
		local position = Position(pos.x, pos.y, pos.z)
		position:sendMagicEffect(effect)
	end
end

if not isItemInPosition then
	function isItemInPosition(pos, itemId)
		local thing = findItemInPosition(pos, itemId)
		return thing ~= nil and thing.uid ~= nil and thing.uid > 0
	end
end

if not transformItemInPosition then
	function transformItemInPosition(pos, fromId, toId)
		return Game.transformItemInPosition(pos, fromId, toId)
	end
end

if not removeItemInPosition then
	function removeItemInPosition(pos, itemId)
		return Game.removeItemInPosition(pos, itemId)
	end
end

if not onUseQuest then
	function onUseQuest(player, item, chest)
		...
		if mainItemType:isStackable() and (chest.item.count or 0) > 1 then
		...
		local rewardWeight = mainItemType:getWeight(chest.item.count or 1)
		if chest.content then
			for _, reward in ipairs(chest.content) do
				local itemType = ItemType(reward.id)
				rewardWeight = rewardWeight + itemType:getWeight(reward.count or 1)
			end
		end
		...
		local noCapacityMessage = string.format(
			"You have found %s. Weighing %d.%02d oz %s too heavy.",
			rewardName, rewardWeight / 100, rewardWeight % 100, term
		)
...
function BridgeRelocate(fromPos, toPos)
	...
	fromTile:relocateTo(Position(toPos.x, toPos.y, toPos.z))

	local stackpos = fromPos.stackpos or 1
	local thing = fromTile:getThing(stackpos)
