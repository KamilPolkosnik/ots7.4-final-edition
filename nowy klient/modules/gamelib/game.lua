...
function g_game.updateFrame(widget, item)
  local frame = widget:getChildById("frame")
  if not item then
    frame:hide()
    return true
  end
  ...
  local frameSource
  local rarityId = item:getRarityId()
  if rarityId == NONE then
    -- frameSource = "0 0 32 32"
    frame:hide()
    return true
  elseif rarityId == COMMON then
    frameSource = "0 0 32 32"
  elseif rarityId == RARE then
    frameSource = "32 0 32 32"
  elseif rarityId == EPIC then
    frameSource = "96 0 32 32"
  elseif rarityId == LEGENDARY then
    frameSource = "128 0 32 32"
  end

  frame:show()
  frame:setImageClip(frameSource)
end
