...
function resolveUseWithTarget(clickedWidget, mousePosition)
  if clickedWidget:getClassName() == 'UIGameMap' then
    ...
    if tile then
      if selectedThing:isFluidContainer() or selectedThing:isMultiUse() then
        ...
        return tile:getTopMultiUseThingEx(clickedWidget:getPositionOffset(mousePosition))
      end
      return tile:getTopUseThing()
    end
...
function onUseWith(clickedWidget, mousePosition)
  local targetThing = resolveUseWithTarget(clickedWidget, mousePosition)
  if targetThing then
    g_game.useWith(selectedThing, targetThing, selectedSubtype)
  end
end
...
function startUseWith(thing, subType, callback)
  ...
  if g_ui.isMouseGrabbed() then
    if selectedThing then
      selectedThing = thing
      selectedType = 'use'
      selectedUseWithCallback = callback
    end
    return
  end
  selectedType = 'use'
